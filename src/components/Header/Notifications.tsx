...
const NotificationsContent: React.FC<NotificationsProps> = ({
  hasNotificationsNotVisualized,
  updateNotifications,
  notifications,
}) => {
  return (
    <Notifications
      hasNotificatons={hasNotificationsNotVisualized}
      className="rounded-xl dropdown dropdown-end  "
    >
      ...
      <ul
        tabIndex={0}
        className="rounded-xl dropdown-content menu  bg-base-100 w-60"
      >
        <div>
            ...
            {!!notifications?.length ? (
              notifications?.map((notification) => {
                return (
                  <div
                    key={notification?.id}
                    className="p-3 border-gray-200 border-b-2 "
                  >
                    <div className="w-full justify-between flex gap-2 items-center mb-3">
                      <p className="text-base font-bold">
                        <>{notification.solicitation?.status}</>
                      </p>
                      <span className="text-gray-300 text-sm ">
                        <>
                          {moment(notification.solicitation?.createdAt).format(
                            "DD/MM - H"
                          ) + "h"}
                        </>
                      </span>
