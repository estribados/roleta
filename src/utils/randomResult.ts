  ...
  const aleatorio = data.map((item,index) =>{
    var percentual = (Math.floor((Math.random() * 100)) + 1);
      if(item.percentageQuota){
        var value = ((Number(item.percentageQuota) >= percentual) ?
        {pass:true,position:index,value:item.valueQuota,percentageQuota:item.percentageQuota} :
        {pass:false,position:index,value:item.valueQuota,percentageQuota:item.percentageQuota});
        return value
      }
    })


    const pass = aleatorio.find((i) =>{return i?.pass})
    let passeds = aleatorio.filter((i) =>{return i?.pass})
    ...
    //pegar o menor item
     ...
     const maxValue = passeds.reduce(function(prev, current) { 
      return Number(prev?.percentageQuota) > Number(current?.percentageQuota) ? prev : current; 
    })
